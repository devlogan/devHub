...
const CreateProfile = ({
  ...
  const profile = profileObj.profile ? profileObj.profile : {};
  ...
  const currentInitialState = profile
    ? { ...updateProfileInitialState }
    : { ...createProfileInitialState };

  const [formData, setFormData] = useState({ ...currentInitialState });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      createProfile(formData, history, !!profileObj.profile);
    } catch (err) {
      console.log(err);
    }
  };
